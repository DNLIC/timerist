# Firebase Test Links

**Base URL:** https://timerist-4ecd9.web.app

## Next.js Pages

1. **Main Demo Page**
   - https://timerist-4ecd9.web.app/
   - https://timerist-4ecd9.web.app/index.html

2. **Timer Example Page**
   - https://timerist-4ecd9.web.app/timer-example
   - https://timerist-4ecd9.web.app/timer-example/

## HTML Timer Pages

3. **Main Timer Index (HTML)**
   - https://timerist-4ecd9.web.app/index.html

4. **Quick Countdown**
   - https://timerist-4ecd9.web.app/quick-countdown.html

5. **Quick Interval iPhone (shadcn)** — mobile testing
   - https://timerist-4ecd9.web.app/quick-interval-iphone-shadcn.html

6. **Tabata Timer**
   - https://timerist-4ecd9.web.app/tabata.html

7. **Interval Timer**
   - https://timerist-4ecd9.web.app/interval.html

8. **Countdown Timer**
   - https://timerist-4ecd9.web.app/countdown.html

9. **Stopwatch**
   - https://timerist-4ecd9.web.app/stopwatch.html

10. **EMOM Timer**
    - https://timerist-4ecd9.web.app/emom.html

11. **Custom Timer**
    - https://timerist-4ecd9.web.app/custom.html

## Quick Copy List (All Links)

```
https://timerist-4ecd9.web.app/
https://timerist-4ecd9.web.app/timer-example
https://timerist-4ecd9.web.app/index.html
https://timerist-4ecd9.web.app/quick-countdown.html
https://timerist-4ecd9.web.app/quick-interval-iphone-shadcn.html
https://timerist-4ecd9.web.app/tabata.html
https://timerist-4ecd9.web.app/interval.html
https://timerist-4ecd9.web.app/countdown.html
https://timerist-4ecd9.web.app/stopwatch.html
https://timerist-4ecd9.web.app/emom.html
https://timerist-4ecd9.web.app/custom.html
```

## Testing Checklist

- [ ] Test on iPhone 13 (390px portrait)
- [ ] Test on iPad (768px)
- [ ] Test on Desktop (1920px+)
- [ ] Test dark mode toggle on all pages
- [ ] Test timer functionality on each page
- [ ] Verify responsive layouts don't collapse
- [ ] Check touch targets are 44x44px minimum
- [ ] Test all timer modes (Tabata, Interval, EMOM, etc.)
